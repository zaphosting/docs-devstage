...
يمكنك العثور على SteamID64 الخاص بك من خلال الذهاب إلى ملفك الشخصي في Steam والنقر بزر الفأرة الأيمن في أي مكان فيه. بعدها اضغط على **نسخ رابط Steam**. 
...
هذا سيعطيك معلومات عامة بالإضافة إلى Steam ID الخاص بحسابك. في حالتنا نحتاج فقط إلى SteamID64. يتم تحديد SteamID64 تحت ``<admins>...</admins>``. سيكون الشكل كالتالي:
...
تأكد من إزالة رموز التعليق `<!--` و `-->` لجعل السطر صالحًا. وإلا سيبقى السطر مجرد تعليق ولن يتم تطبيقه. ببساطة قم بحذف هذه الرموز من بداية ونهاية السطر لتفعيله.
:::
...
يمكن تعريف صلاحيات جميع أوامر الأدمن تحت ``permissions``. لهذا، يجب تعديل ``permission_level`` تمامًا كما عند إضافة الأدمنز. سيكون الشكل كالتالي:
...
مستوى الصلاحية هو قيمة بين 0 و 1000 ويحدد الصلاحيات التي يمتلكها اللاعب. 1000 هو الأدنى (بدون صلاحيات) و0 هو الأعلى (صلاحيات أدمن كاملة). بناءً على كيف تريد أن تكون الصلاحيات، يجب تعديل هذا الإعداد وفقًا لذلك. 
