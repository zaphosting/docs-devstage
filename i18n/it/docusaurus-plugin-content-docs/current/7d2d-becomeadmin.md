...
Qui otterrai info generali e lo Steam ID del tuo account. A noi serve solo lo SteamID64. Lo SteamID64 va inserito sotto ``<admins>...</admins>``. Avrà questo aspetto:
...
Il gioco permette di definire diversi livelli di permessi per gli amministratori. Questo significa che puoi creare gruppi admin con permessi diversi. Il livello si imposta con l’opzione ``permission_level``, che va da 0 a 1000. A seconda di come lo configuri, gli admin avranno accesso ai permessi assegnati. Una volta fatto, i permessi admin sono stati assegnati con successo. 
...
Il livello di permesso è un valore tra 0 e 1000 che determina cosa può fare un giocatore. 1000 è il più basso (nessun permesso), 0 è il più alto (permessi admin completi). Regola i livelli in base a come vuoi gestire i permessi. 
