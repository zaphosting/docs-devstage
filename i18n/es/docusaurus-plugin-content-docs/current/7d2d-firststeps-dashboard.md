...
Al hacer clic en la imagen, puedes crear tu propia tienda para servidor de juegos con Tebex. La configuración es rápida y sencilla, para que puedas empezar a vender tus bienes virtuales en nada de tiempo.
...
¿Tu servidor estuvo offline temporalmente y no sabes por qué? ¿Revisaste el registro de eventos y archivos de registro y no encontraste nada? Puede que la causa no esté directamente en tu servidor, sino en el sistema host donde se ejecuta.
...
En esta sección encontrarás un historial de interrupciones del sistema host donde corre tu servicio de servidor de juegos. Más info y la versión más actualizada del estado de nuestros servicios la encuentras en nuestra nueva [página de estado](https://status.zap-hosting.com/).
...
Con el editor tienes acceso directo a todas las opciones disponibles y puedes configurarlas con precisión. Esto es especialmente útil para usuarios avanzados que quieren control total sobre los ajustes y saben exactamente qué cambiar.
...
**FileZilla:** FileZilla es el cliente FTP más fácil y conocido. Solo instálalo, ábrelo e ingresa tus datos de acceso en la barra superior, que encuentras un poco más arriba en esta página. Con un clic en "Conectar" accedes directamente al servidor y puedes gestionar los archivos.
...
En los ajustes puedes definir si las copias se crean automáticamente y si es diario o semanal. Además, puedes decidir si se eliminan las copias más antiguas cuando sea necesario, por ejemplo si el espacio de almacenamiento se limita.
