...
你可以通过进入你的Steam个人资料，右键点击页面任意位置，然后点击 **复制Steam链接** 来找到你的SteamID64。
...
:::danger  管理员记录未被识别？ 
确保删除注释符号 `<!--` 和 `-->`，使该行生效。否则这行会被当作注释忽略。只需删除行首和行尾的注释符号即可激活该条目。
:::

游戏支持为管理员权限定义不同的权限等级，也就是说可以创建不同权限的管理员组。权限等级由 ``permission_level`` 选项定义，范围是0到1000。根据设置，管理员将拥有对应的权限。完成后，管理员权限即成功分配。 
