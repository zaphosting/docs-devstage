...
description: "Découvrez comment gérer efficacement votre serveur de jeux 7 Days to Die et optimiser ses réglages pour une expérience de jeu au top → En savoir plus maintenant"
...
Dans les **Statistiques**, tu as une vue complète sur les valeurs de charge de ton serveur. Tu y trouveras des infos sur différents aspects comme la charge des slots disponibles, le CPU et la mémoire. Tu peux consulter ces données pour différentes périodes, par exemple les dernières **60 minutes, 24 heures, 3 jours, 7 jours** ou **30 jours.**

![](https://screensaver01.zap-hosting.com/index.php/s/N8jcmdqqG2Xt4Bk/preview)

Grâce à ces infos, tu as un bon aperçu de l’utilisation de ton serveur. En plus, ces statistiques sont souvent utiles pour identifier d’éventuels soucis de performance et en cerner les causes. Par exemple, tu peux retracer quand l’utilisation a augmenté et si ça a un lien avec certains événements ou changements. C’est super important quand des problèmes soudains arrivent, car ça te permet de prendre des mesures ciblées pour dépanner et analyser méthodiquement les causes possibles.
...
En cliquant sur l’image, tu peux créer ta propre boutique de serveur de jeux avec Tebex. La mise en place est rapide et facile, tu seras prêt à vendre tes biens virtuels en un rien de temps.
...
Ton serveur a été hors ligne temporairement et tu ne sais pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers journaux n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de ton serveur, mais du système hôte sur lequel il tourne.
...
En plus, tu peux aussi activer la notification par email pour les mises à jour automatiques, pour rester informé et toujours à la page.
